...
import com.plovdev.pronviewer.events.listeners.FileListener;
import com.plovdev.pronviewer.gui.filters.FilterBox;
import com.plovdev.pronviewer.models.DownloadedVideoCard;
import com.plovdev.pronviewer.models.DownloadingVideoCard;
...
public class DownloadsPane extends AnchorPane {
    ...
    public DownloadsPane() {
        ...
        FileListener fileListener = new FileListener("downloads");
    ...
    private Runnable getParseTask(FlowPane pane) {
        return () -> {
            try (Stream<Path> stream = Files.walk(Path.of("downloads"))) {
                ...
                List<DownloadedVideoCard> cards = paths.stream().filter(e -> e.toString().endsWith(".mp4")).map(p -> {
                    ...
                    try {
                        ...
                        card.setDeleteRun(() -> {
                            isSelf = card.isSelf();
                            try {
                                Files.delete(Path.of(card.getPath().substring(card.getPath().indexOf(':') + 1)));
